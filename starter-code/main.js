...
mongoClient.connect(`mongodb://127.0.0.1:27017/crunchbase`, (error, db) => {
      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ founded_year: 2004 }, { founded_month: { $lte: 6 , $gte: 4} }] }, { name: 1, _id: 0 }).sort({ founded_day: 1, founded_month: 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ $and: [{name: "Facebook"}, { "relationships.is_past": false }] }, {"relationships.person": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ $and: [{tag_list: {$regex: "social-networking"}}, { founded_year: { $lte: 2016 , $gte: 2002} }] }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ $and: [{tag_list: {$regex: "social-networking"}}, { founded_year: { $lte: 2016 , $gte: 2002}}, {"offices.city": "London"} ] }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
